-- SaveManager.lua - OBSIDIAN LIBRARY ADDON
-- Config save/load system with auto-load support
...
if typeof(clonefunction) == "function" then
    ...
    local success, _ = pcall(function()
        return isfolder_copy("test_" .. tostring(math.random(1000000, 9999999)))
    end)

    if success == false or type(success) ~= "boolean" then
        isfolder = function(folder: string): boolean
            local s, r = pcall(isfolder_copy, folder)
            return s == true and r == true
        end

        isfile = function(file: string): boolean
            local s, r = pcall(isfile_copy, file)
            return s == true and r == true
        end

        listfiles = function(folder: string): { string }
            local s, r = pcall(listfiles_copy, folder)
            if s == true and type(r) == "table" then
                return r
            end
            return {}
        end
